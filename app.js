...
index 0000000000000000000000000000000000000000..be1202c549d560a4af77a06cdf03e828c71bcb86
--- /dev/null
+++ b/app.js
@@ -0,0 +1,401 @@
+const archetypes = [
...
+    if (
+      /^(An item to be held by a Pokémon\.|One of a variety of mysterious Mega Stones\.|If a Pokémon holds one of these Berries|If a Pokémon holding this Berry)/.test(next) &&
+      !/^(Picture|Name|Effect|Location|Shop|Beginning|Mega Evolution Tutorial|Deposit)/.test(candidate)
+    ) {
+      names.push(candidate.replace(/\s+/g, ' ').trim());
+    }
+  }
+
...
+}
+
+function parseRawPokemon(rawText) {
+  const matches = [...rawText.matchAll(/Image\s*\n([^\n]+)\n/g)];
+  const names = matches.map((m) => m[1].trim()).filter((name) => name && !name.includes('Image'));
+  return [...new Set(names)];
+}
+
+function parseRawDataBlob() {
+  const raw = byId('rawDataInput').value || '';
+  const status = byId('rawParseStatus');
+  const itemsList = byId('rawItemsList');
+  const pokemonList = byId('rawPokemonList');
+  const wrap = byId('rawParsedWrap');
+
+  if (!raw.trim()) {
+    status.textContent = 'Paste raw text first.';
+    wrap.classList.add('hidden');
+    return;
+  }
+
+  const items = parseRawItems(raw);
+  const pokemon = parseRawPokemon(raw);
+
+  itemsList.innerHTML = '';
+  pokemonList.innerHTML = '';
+
+  items.slice(0, 80).forEach((name) => {
+    const li = document.createElement('li');
+    li.textContent = name;
+    itemsList.appendChild(li);
+  });
+
+  pokemon.slice(0, 200).forEach((name) => {
+    const li = document.createElement('li');
+    li.textContent = name;
+    pokemonList.appendChild(li);
+  });
+
+  status.textContent = `Parsed ${items.length} items and ${pokemon.length} Pokémon names from pasted raw text.`;
...
+byId('parseRawBtn').addEventListener('click', parseRawDataBlob);
+
+renderAll();
