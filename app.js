...
index 0000000000000000000000000000000000000000..7442e89f3d94221a618655c3a93e814bc3a8297f
--- /dev/null
+++ b/app.js
@@ -0,0 +1,473 @@
+const archetypes = [
...
+};
+
+function convertDexToPool(dexMap) {
+  return Object.entries(dexMap).map(([name, entry]) => {
+    const stats = Array.isArray(entry.baseStats) ? entry.baseStats : [80, 80, 80, 80, 80, 80];
+    const [hp, atk, def, spa, spd, spe] = stats.map((value) => Number(value) || 80);
+    return {
+      name,
+      types: Array.isArray(entry.types) ? entry.types : ['Normal'],
+      role: 'Imported Pokémon Champions entry',
+      tierScore: Math.round((atk + def + spa + spd + spe) / 5),
+      archetypes: ['Hyper Offense', 'Balanced', 'Bulky Offense', 'Stall', 'Rain'],
+      baseStats: { hp, atk, def, spa, spd, spe },
+      legalMoves: Array.isArray(entry.legalMoves) ? entry.legalMoves : []
+    };
+  });
+}
+
+const pokemonPool = convertDexToPool(pokemonDex);
+
+
+const typeChart = {
...
+    metaTeams: [],
+    tiers: {},
+    importedPokemonDex: null
+  }
+};
+
+function byId(id) { return document.getElementById(id); }
+
+function getPokemonDataset() {
+  return state.external.importedPokemonDex?.length ? state.external.importedPokemonDex : pokemonPool;
+}
+
...
+    const opponent = [...getPokemonDataset()].sort(() => Math.random() - 0.5).slice(0, 6);
...
+  const dataset = getPokemonDataset();
...
+  const dataset = getPokemonDataset();
+  const attacker = dataset.find((mon) => mon.name === byId('attacker').value);
+  const defender = dataset.find((mon) => mon.name === byId('defender').value);
...
+}
+
+function parseImportBlob(rawText) {
...
+  const normalized = wrapped.replace(/,\s*([}\]])/g, '$1');
+  return JSON.parse(normalized);
+}
+
+function convertImportedPokemon(rawMap) {
+  return convertDexToPool(rawMap).map((entry) => ({
+    ...entry,
+    role: 'Imported from your Pokémon Champions dex'
+  }));
+}
+
+function importPokemonDex() {
+  const status = byId('pokemonDexStatus');
+  const selectIds = ['attacker', 'defender'];
+
+  try {
+    const raw = byId('pokemonDexInput').value || '';
+    const parsed = parseImportBlob(raw);
+    const imported = convertImportedPokemon(parsed);
+    if (!imported.length) {
+      throw new Error('Parsed zero Pokémon entries.');
+    }
+
...
+      byId(id).innerHTML = '';
+    });
+    status.textContent = `Imported ${imported.length} Pokémon entries. Team builder/sim/calc now use your imported dex.`;
...
+byId('importPokemonDexBtn').addEventListener('click', importPokemonDex);
+
+renderAll();
